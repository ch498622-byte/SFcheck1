...
import { FeeStandardRow } from '../types';

interface Props {
  isOpen: boolean;
  onClose: () => void;
  rules: FeeStandardRow[];
  onSave: (rules: FeeStandardRow[]) => void;
}

const SFFeeConfigModal: React.FC<Props> = ({ isOpen, onClose, rules, onSave }) => {
  const [localRules, setLocalRules] = useState<FeeStandardRow[]>(rules);
  ...
  const handleChange = (index: number, field: keyof FeeStandardRow, value: string) => {
    const newRules = [...localRules];
    if (field === 'startProvince' || field === 'endProvince') {
  ...
  const addRule = () => {
    setLocalRules([...localRules, { startProvince: '', endProvince: '', priceUnder05: 0, price05To1: 0, priceStep: 0 }]);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fade-in backdrop-blur-sm">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-5xl mx-4 max-h-[85vh] flex flex-col transform transition-all scale-100">
        <div className="flex justify-between items-center p-6 border-b border-gray-100">
          <div>
             <h3 className="text-xl font-bold text-gray-900">顺丰运费配置</h3>
             <p className="text-xs text-gray-500 mt-1">配置各流向的首重与续重价格标准 (0.5kg 阶梯)</p>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-3 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">0.5kg内费用 (元)</th>
                <th className="px-3 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">0.5-1kg费用 (元)</th>
                <th className="px-3 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">续重(0.5kg) (元)</th>
              ...
              {localRules.map((rule, idx) => (
                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-3 py-3">
                    <input 
                      type="text" 
                      value={rule.endProvince}
                      onChange={(e) => handleChange(idx, 'endProvince', e.target.value)}
                      className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm border p-1.5"
                      placeholder="如: 广东"
                    />
                  </td>
                  <td className="px-3 py-3">
                     <input 
                      ...
                      value={rule.priceUnder05}
                      onChange={(e) => handleChange(idx, 'priceUnder05', e.target.value)}
                      className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm border p-1.5"
                    />
                  </td>
                  <td className="px-3 py-3">
                    <input 
                      type="number" step="0.1"
                      value={rule.price05To1}
                      onChange={(e) => handleChange(idx, 'price05To1', e.target.value)}
                      className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm border p-1.5"
                    />
                  </td>
                  <td className="px-3 py-3">
                    <input 
                      ...
                      onChange={(e) => handleChange(idx, 'priceStep', e.target.value)}
                      className="block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 text-sm border p-1.5"
                    />
                  </td>
...
export default SFFeeConfigModal;
